...
string DES(){ 
	...
	cout<<"Expansion Box 32 to 48 : "<<right_expanded<<endl;    	
	// 3.3. The result is xored with a key
		string xored = Xor(round_keys[i], right_expanded);  
		cout<<"K"<<i<<" XOR E(R"<<i<<") : "<<xored<<endl;
		string res = ""; 
		...
		for(int i=0;i<8; i++){ 
			...
			// substituition box
      		string row1= xored.substr(i*6,1) + xored.substr(i*6 + 5,1);  //00
      		int row = convertBinaryToDecimal(row1);						//0 row 
      		string col1 = xored.substr(i*6 + 1,1) + xored.substr(i*6 + 2,1) + xored.substr(i*6 + 3,1) + xored.substr(i*6 + 4,1);  //1001
			int col = convertBinaryToDecimal(col1);   //9
			int val = substition_boxes[i][row][col];
			res += convertDecimalToBinary(val);  
		} 
		cout<<"Substitution box result 48 to 32 : "<<res<<endl;
		// 3.5. Another permutation is applied
		string perm2 =""; 
		for(int i = 0; i < 32; i++){ 
			perm2 += res[permutation_tab[i]-1]; 
		}
		cout<<"P box permutation on 32 :"<<perm2<<endl;
		// 3.6. The result is xored with the left half
		xored = Xor(perm2, left);
		// 3.7. The left and the right parts of the plain text are swapped 
			cout<<"Left" <<i+1<<" : "<<right<<"\t"<<"Right"<<i+1<<" : "<<xored<<endl<<endl;
		left = xored; 
		if(i < 15){ 
			string temp = right;
			right = xored;
	...
	cout<<"Combined Text : "<<combined_text<<endl;
	string ciphertext =""; 
	// The inverse of the initial permuttaion is applied
	for(int i = 0; i < 64; i++){ 
		ciphertext+= combined_text[inverse_permutation[i]-1]; 
			//cout<<"Ciphertext:1 "<<ciphertext<<endl;
	}
	cout<<"Apply inverse Permutation: "<<ciphertext<<endl;
...
int main(){ 
	// A 64 bit key
//	string key= "0101100000011111101111001001010011010011101001000101001011101010";
string key= "0101100000011111101111001001010011010011101001000101001011101010";
//string key= "01011000001111101111010010101101001101001001010011110101";
//  string key= "01011000001111101111010010101101001101001001010011110101";


	// A block of plain text of 64 bits
	//MESSAGE = 3570 E2F1 BA46 82C7HEX 						

//	pt= "0011010101110000111000101111000110111010010001101000001011000111";
pt="0010001001100100101010111000010110100011110010111001111100111000";
	string apt = pt;
	// Calling the function to generate 16 keys
  	generate_keys(key); 
    cout<<"Plain text: "<<pt<<endl; 
	// Applying the algo
	//string ct="0011010101110000111000101111000110111010010001101000001011000111";
   string ct= DES(); 
...
//2264AB85A3CB9F38 plain text
//3570E2F1BA4682C7 cipher text 
